...
def calc_metrics(img_out, img_gt, args):
    ...
    for index in range(img_gt.size(0)):
        sum_psnr += torch_psnr(
            img_out[index : index + 1], img_gt[index : index + 1], border=6, data_range=args.data_range  # 默认去除6个边界像素
        )
...
@master_only
def set_checkpoint_and_log_dir(args):
    if args.test_only:
        return False
    create_dir("./logs/{}/{}".format(args.dataset, args.exp_name))
    create_dir("./checkpoints/{}/{}".format(args.dataset, args.exp_name))
