...
def train_one_epoch(model, criterion, train_data, optimizer, device, epoch, args):
    ...
    for samples in metric_logger.log_every(iterable=train_data, print_freq=args.print_freq, header=header):
...
def evaluate(model, criterion, test_name, val_data, device, args):
    ...
    for _, samples in val_data:
        samples = utils.to_device(samples, device)
        out = utils.ensemble(samples, model, "mean", args.dataset)
        torch.cuda.synchronize()
        if test_name == "val":
            loss = criterion(out["img_out"], samples["img_gt"])
    ...
    if test_name == "val":
        ...
        metric_out = {
            "{}_".format(test_name) + k: round(meter.global_avg, 3) for k, meter in metric_logger.meters.items()
        }
        print(metric_out)
        return metric_out
